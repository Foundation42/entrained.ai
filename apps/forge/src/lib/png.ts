...
// Adapted from apps/sprites for single-image transparency masking
...
export async function mergeWithMask(
  ...
  for (let i = 0; i < width * height; i++) {
    ...
    // Get RGB from image
    result[srcOffset] = image.pixels[srcOffset];     // R
    result[srcOffset + 1] = image.pixels[srcOffset + 1]; // G
    result[srcOffset + 2] = image.pixels[srcOffset + 2]; // B
